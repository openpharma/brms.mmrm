...
#' @export
#' @family marginals
#' @description Average marginal MCMC draws across time points.
...
#' )
#' }
brm_marginal_draws_average <- function(draws, data, times = NULL) {
  assert(
    is.list(draws),
...
brm_marginal_draws_average_df <- function(draws, levels_group, levels_time) {
  ...
  for (group in levels_group) {
    ...
    if (length(names) > 0L) {
      ...
      draws[[group]] <- apply(X = subset, MARGIN = 1L, FUN = mean)
    }
  }
