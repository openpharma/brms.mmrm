...
#'
#'   If you supply a `mice` multiply imputed dataset in the `imputed`
...
#' @param imputed Either `NULL` (default) or an optional multiply imputed
#'   dataset ("mids") object generated by [mice::mice()]. If not `NULL`,
#'   then the `data` argument is ignored and the MMRM is fit
#'   successively to each dataset in `imputed` using [brms::brm_multiple()].
#'   Posterior draws are combined automatically for downstream post-processing
...
brm_model <- function(
  ...
  assert(
    imputed,
    is.null(.) || inherits(., "mids"),
    message = paste(
      "argument 'imputed' must either be NULL or a {mice} multiply",
      "imputed dataset (object of class 'mids')."
    )
  )
  modeled_data <- if_any(
