test_that("brm_archetype_average_cells() change and non-subgroup", {
  set.seed(0L)
  data <- brm_simulate_outline(
    ...
    dplyr::mutate(response = rnorm(n = dplyr::n()))
  out <- brm_archetype_average_cells(
    data,
    prefix_interest = "y_",
    prefix_nuisance = "z_",
    baseline = TRUE,
  ...
  out2 <- brm_archetype_average_cells(
    out,
    prefix_interest = "y_",
    prefix_nuisance = "z_",
    baseline = TRUE,
  ...
  expect_s3_class(out, "brms_mmrm_average_cells")
  ...
  exp <- kronecker(diag(2L), exp)
  expect_equal(unname(as.matrix(grid[, seq(3L, 8L)])), exp)
})

test_that("brm_archetype_average_cells() non-change subgroup", {
  set.seed(0L)
  data <- brm_simulate_outline(
    n_group = 2,
    n_subgroup = 3,
    ...
    dplyr::mutate(response = rnorm(n = dplyr::n()))
  out <- brm_archetype_average_cells(data)
  out2 <- brm_archetype_average_cells(out)
  expect_equal(out, out2)
  expect_silent(brm_data_validate(out))
  expect_true(all(class(data) %in% class(out)))
  expect_s3_class(out, "brms_mmrm_average_cells")
  ...
  exp <- diag(3L)
  exp[1L, c(1L, 2L, 3L)] <- c(3L, -1L, -1L)
  exp <- kronecker(diag(6L), exp)
  expect_equal(unname(as.matrix(grid[, seq(4L, 21L)])), exp)
})

test_that("brm_archetype_average_cells() non-subgroup intercept", {
  set.seed(0L)
  data <- brm_simulate_outline(
    n_group = 2,
    n_patient = 100,
...
test_that("brm_archetype_average_cells() subgroup intercept", {
  set.seed(0L)
  data <- brm_simulate_outline(
    n_group = 2,
    n_subgroup = 3,
    ...
    dplyr::mutate(response = rnorm(n = dplyr::n()))
  out <- brm_archetype_average_cells(data, intercept = TRUE)
  grid <- dplyr::distinct(
    out,
    group,
    subgroup,
