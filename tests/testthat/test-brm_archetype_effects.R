test_that("brm_archetype_effects() change and non-subgroup", {
  set.seed(0L)
  ...
  expect_s3_class(out, "brms_mmrm_effects")
  ...
  expect_equal(
    ...
    kronecker(rbind(c(1, 0), c(1, 1)), diag(3L))
  )
})

test_that("brm_archetype_effects() non-change subgroup", {
  set.seed(0L)
  data <- brm_simulate_outline(
    n_group = 2,
    n_subgroup = 3,
    ...
    dplyr::mutate(response = rnorm(n = dplyr::n()))
  out <- brm_archetype_effects(data)
  out2 <- brm_archetype_effects(out)
  expect_equal(out, out2)
  expect_silent(brm_data_validate(out))
  expect_true(all(class(data) %in% class(out)))
  expect_s3_class(out, "brms_mmrm_effects")
  ...
  expect_equal(
    ...
    kronecker(rbind(c(1, 0), c(1, 1)), diag(9L))
  )
})

test_that("brm_archetype_effects() intercept non-subgroup", {
  set.seed(0L)
  data <- brm_simulate_outline(
    n_group = 2,
    n_patient = 100,
  ...
  expect_equal(grid$group, rep(c("group_1", "group_2"), each = 3L))
  expect_equal(grid$time, rep(paste0("time_", c(2L, 3L, 4L)), times = 2L))
  exp <- kronecker(rbind(c(1, 0), c(1, 1)), diag(3L))
  exp[, 1L] <- 1L
  expect_equal(
    unname(as.matrix(grid[, seq(3L, 8L)])),
    exp
...
test_that("brm_archetype_effects() intercept subgroup", {
  set.seed(0L)
  data <- brm_simulate_outline(
    n_group = 2,
    n_subgroup = 3,
    ...
    dplyr::mutate(response = rnorm(n = dplyr::n()))
  out <- brm_archetype_effects(data, intercept = TRUE)
  grid <- dplyr::distinct(
    out,
    group,
    subgroup,
  ...
  expect_equal(grid$time, rep(paste0("time_", seq_len(3L)), times = 6L))
  exp <- kronecker(rbind(c(1, 0), c(1, 1)), diag(9L))
  exp[, 1L] <- 1L
  expect_equal(
    unname(as.matrix(grid[, seq(4L, 21L)])),
    exp
