...
map_mix <- RBesT::automixfit(map_mcmc)

map_prior <- RBesT::robustify(
  map_mix,
  weight = 0.2,
  mean = 0,
...
```{r}
# 8 coefficients in the order brms assigns to class = "b":
#   b[1:4] = x_PBO_VIS1, x_PBO_VIS2, x_PBO_VIS3, x_PBO_VIS4
#   b[5:8] = x_TRT_VIS1, x_TRT_VIS2, x_TRT_VIS3, x_TRT_VIS4

# Informative component: informed PBO means, vague TRT effects.
mean_informative <- c(5, 7, 8, 9, 0, 0, 0, 0)

# Block-diagonal covariance: correlated PBO block, vague diagonal TRT block.
cov_pbo <- matrix(
  c( 4, 2, 1, 1,
     2, 4, 2, 1,
     1, 2, 4, 2,
...
cov_informative <- matrix(0, 8, 8)
cov_informative[1:4, 1:4] <- cov_pbo
cov_informative[5:8, 5:8] <- diag(rep(100, 4))

# Robust component: vague everywhere.
mean_robust <- rep(0, 8)
cov_robust <- diag(rep(100, 8))

multivariate_mixture <- RBesT::mixmvnorm(
  informative = c(0.8, mean_informative, cov_informative),
  robust = c(0.2, mean_robust, cov_robust),
  sigma = diag(rep(100, 8))
)
