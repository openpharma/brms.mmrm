...
packages <- c("tidyr", "dplyr", "stringr", "tibble", "ggplot2", "gt", "gtsummary")
...
cdystonia1 |> select(treat, age, sex, baseline) |> distinct() |>
  tbl_summary(
    by = c(treat),  
    statistic = list(
...
unique(cdystonia1$week) |> sort() |>
  ...
  tbl_stack(quiet = T) |>
  modify_caption("**Change from baseline in total TWSTRS score.**")
```
...
cdystonia1 |> group_by(treat) |>
  ggplot(aes(x = week, y = delta, fill = factor(treat))) +
  geom_boxplot() + 
  labs(
    x = "Visit", 
