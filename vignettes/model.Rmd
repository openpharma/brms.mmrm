...
$$
\begin{aligned}
&y_i \stackrel{\text{ind}}{\sim} \text{Multivariate-Normal} \left (\text{mean} = X_{i} \beta, \ \text{covariance} = \text{diag}(Z_i\sigma) \cdot \Lambda \cdot \text{diag}(Z_i\sigma) \right ) \\
...
* $X_i$: the rows of the fixed effect model matrix which correspond to subject $i$. The rows of $X_i$ correspond to the elements of the vector $y_i$ (equivalently, the discrete time points of the study), and the columns of $X_i$ correspond to the model coefficient parameters $\beta_p$. The value of $X_i$ is determined by the data and by the fixed effect parameterization declared through `brm_formula()`.
* $Z_i$: the rows of the standard deviation model matrix which correspond to subject $i$. The rows of $Z_i$ correspond to the elements of the vector $y_i$ (equivalently, the discrete time points of the study), and the columns of $Z_i$ correspond to the scalar elements of $\sigma$. The value of $Z_i$ is determined by the data and by the fixed effect parameterization declared through `brm_formula()`.
